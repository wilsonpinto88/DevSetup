...
- `AgentSetup\` -- snapshot of AI agent config (29 Copilot skills, 25 prompts, instructions, Claude Code CLAUDE.MD/settings). See `AgentSetup\README.md` for details. This is a one-way restore only (repo → machine) -- there's no export script, so update this folder manually if you change your skills/prompts later.

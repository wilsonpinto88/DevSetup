...
- **`.github/hooks/context-mode.json`** (repo-level, DevSetup only) -- wires the `context-mode` plugin's native VS Code Copilot integration (`sessionStart`/`preToolUse`/`postToolUse`/`preCompact`) into this repo, giving Copilot the same context-window-protection sandboxing (`ctx_execute`, `ctx_search`) that Claude Code already has via the plugin. Requires `.vscode/mcp.json` (also in this repo) pointing at the globally-installed `context-mode` command, and `npm install -g context-mode`.
- **`~/.copilot/hooks/workspace-config-bootstrap.json`** (user-level, applies everywhere, added 2026-09-04) -- fires on `sessionStart` in *any* workspace and auto-creates the two files above if they're missing: adds a `context-mode` entry to `.vscode/mcp.json` (merging with any existing MCP servers rather than clobbering them) and writes `.github/hooks/context-mode.json` from the standard template if the file doesn't exist at all. Also checks whether `context-mode` is on PATH and adds a warning note to session context if not. No `.git` check -- fires in any workspace Copilot opens, per project decision. Never fails closed (always exits 0) so a bootstrap error can't block session start. This makes the per-project wiring effectively global: open any repo in Copilot once and the config is created automatically instead of needing a manual copy.

...
- **Copilot numbers will not match VS Code's native "Copilot Business" widget (or the GitHub billing panel) exactly — there's a margin of error.** Even after the dedup/unbilled-row fix above, local totals landed close but not identical to the real billing panel (observed gap: ours ≈5153 credits vs. the panel's 5255.7 in one real comparison). `session-store.db` is a local telemetry log, not GitHub's billing ledger — some server-side adjustments (proration, retried-but-still-billed calls, rounding) aren't visible locally. Treat this dashboard's Copilot figures as a close local estimate, and the native widget/GitHub billing page as the source of truth for anything that actually matters financially.
- **"Skills Used" is Claude Code only — blocked upstream, not a gap in this extension.** Verified directly against `session-store.db`: `assistant_usage_events` has no skill/tool column, `turns.assistant_response` is an unstructured text blob (no parseable tool-call data), and `forge_trajectory_events` — whose schema (`event_type`, `command`, `event_value`) looks purpose-built for exactly this — exists but has 0 rows; the Copilot CLI isn't writing to it. If the CLI starts populating that table (or adds a skill column to `assistant_usage_events`), the fix here is small: read it in `copilotDb.ts`, drop the Claude-only filter in `aggregator.ts`'s `computeSkillUsage`, and remove this caveat.
